      ...
      >
        ...
        <template v-if="disp.type === 'trait'">
          ...
          <div class="flex items-center gap-1 flex-shrink-0">
            ...
            <span class="text-xs text-gray-200 w-4 text-center">{{
              disp.trait_count || 1
            }}</span>
            <button
              @click="changeTraitCount(index, 1)"
...
function getChampion(id) {
  return championsMap.value[id] || null;
}
function getTrait(id) {
  return traitsMap.value[id] || null;
}
function getItem(id) {
  ...
  return (props.tftData?.champions || [])
    ...
    .filter((c) => {
      ...
      return (
        ...
        c.traits?.some((t) => t.name?.toLowerCase().includes(s))
      );
    })
